    ...
    const fetchReports = async () => {
      try {
        ...
        const transformedReports = data.map((report: any) => ({
          ...
          status: report.status === 'Pending' ? 'submitted' : (report.status === 'In Progress' ? 'in-progress' : 'resolved'),
          createdAt: report.createdAt,
          imageUrl: report.photoURL ? `http://localhost:5001${report.photoURL}` : null,
          reportedBy: typeof report.reporter === 'object' ? report.reporter._id : report.reporter,
          reporterName: typeof report.reporter === 'object' ? report.reporter.name : 'Unknown',
          reporterEmail: typeof report.reporter === 'object' ? report.reporter.email : '',
                ...
                <TableCell className="text-right">
                  <ContextMenu>
                    ...
                    <ContextMenuContent>
                      ...
                      <ContextMenuItem onClick={() => navigator.clipboard.writeText(report.id)}>
                        <Copy className="mr-2 h-4 w-4" /> Copy Report ID
                      </ContextMenuItem>
                      <ContextMenuItem onClick={() => handleDeleteReport(report.id)}>
